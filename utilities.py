...
class Utilities:
	...
	def getCookieValue(self, request, key):
		...
		try:
			result = cookie[key] #Read a cookie from request or pending request
		except KeyError:
			return ""
...
##                self.response.headers.add_header(*jar.output().split(u': ', 1))
	...
	def checkLogin(self, request):
		...
		if user:
			...
			cookies = request.request.cookies  #Cookies(request) #Optional arguments: max_age, path, domain, secure, httponly, version, comment
	...
	def getRenterSelect(self):
		...
		for renter in renters:
			emailSelect = emailSelect + "<option value='" + renter.email+ "'>" + renter.name + "</option>"
	...
	def getSaturdaySelects(self, year):
		...
		saturdays = ""
		while y == year:
			saturdays += sDate.strftime("<option value='%m/%d/%Y'>%m/%d/%Y</option>")
			sDate = sDate + span
			#logging.info(year)
			y = sDate.year
			#logging.info(year)
	...
	def createDateHeaderRow(self, startDate, endDate):
		...
		while date <= endDate:
			row += date.strftime("<td class='scheduleCell' align='center' width='120px'>%m/%d</td>")
			#logging.info(date)
			date += week
	...
	def createScheduleRow(self, rentals, startDate, endDate, houseName, request, public="n"):
		...
		while date <= endDate:
			for rental in rentals:
				#logging.info(rental.houseName)					
				if rental.startDate == date:
					...
					if prevRenterName == rental.renterName:
						...
						if public == "y":
							tempRow = "<td class='scheduleCell' bgcolor='" + color + "' width='120px'>&nbsp;</td>"
						else:
							tempRow = "<td class='scheduleCell' bgcolor='" + color + "' width='120px'>" + rental.renterName + "</td>"
	...
	def render(self, template_file, context, handler, login_required=True, path="", public="n"):
		...
		if loggedInUser is not None:
                        logging.info("username = " + loggedInUser + ", email = " + loggedInEmail)
                else:
                        loggedInUser = ""
		...
		if  (loggedInUser == "" or loggedInUser is None) and login_required:
			...
			path = os.path.join(os.path.dirname(__file__), os.path.join('templates', template_file))
			if not context:
				context = {'loggedInUser': loggedInUser, 'loggedInEmail':loggedInEmail}
			else:
