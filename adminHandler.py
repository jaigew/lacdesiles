...
class EditRentalHandler(BaseHandler):
	def get(self):
		...
		for rental in rentals:
			rentals_table = rentals_table + "<tr><td>" + rental.renterName + "</td><td>" + rental.houseName + "</td><td>" + rental.startDate.strftime("%m/%d/%Y") + "</td><td>" + rental.endDate.strftime("%m/%d/%Y") + "</td><td>-</td><td><a href='/admin/editRentals/?a=d&re=" + rental.renterEmail + "&sd=" + rental.startDate.strftime("%m/%d/%Y") + "&h=" + rental.houseName + "'>delete</a></td></tr>"
...
class EditPageHandler(BaseHandler):
	def showPages(self):
		...
		pageList = ""
		for p in page:
			pageList = pageList + "<tr><td>" + str(p.id) + "</td><td>" + p.title + "</td></tr>" 
			
		context ={'page':page, 'addeditstyle':addeditstyle, 'pageList':pageList}
...
class EditSettingsHandler(BaseHandler):
	...
	def post(self):
		...
		settings.enddate = datetime.strptime(self.request.get('enddate'), "%m/%d/%Y")
		settings.put()
...
class AdminIndexHandler(BaseHandler):
	def get(self):
		...
		context = {'admin_body': "Welcome to the Lac Des Iles Administration Page" }
...
class AddRenterHandler(BaseHandler):
	...
	def showRenters(self):
		...
		for renter in renters:
			...
			renterList = renterList + "<a href='../addRenters/?id=" + str(renter.id) + "&email=" + renter.email + "'>Edit</a>&nbsp;-&nbsp;<span style='background-color:#" + renter.color + ";height=20px;width=100px;'>" + renter.name + "</span> - " + renter.email + " - " + renter.color + "<br>"

		
		context = {'renterList':renterList, 'r':r, 'addeditstyle':addeditstyle}
...
class AddRentalHandler(BaseHandler):
	...
	def renderSchedule(self):
		...
		u.render('adminSchedule.html', context, self, False, "rental/schedule/")
	...
	def post(self):
		logging.info("POST to Admin Schedule Add" + self.request.get('startDateSelect'))
