...
class FamilyHandler(BaseHandler):
    ...
    def get(self):
        ...
        while i <= 20:
            gallery = gallery + "<a href='/images/family/old_pics_" + \
                str(i) + ".jpg' rel='lightbox[oldpics]' title=''><img src='/images/family/old_pics_" + str(
                    i) + ".jpg' height='75' alt='' rel='lightbox' border=0/></a>&nbsp;&nbsp;"
...
Email Bill at bmott.top@gmail.com if you have any questions about selection.""")
        ...
        self.response.out.write(
            "Email sent to " + renter_next_email + ".<br><br>")
        self.response.out.write(
            "<a href='/'>Return to the Lac Des Iles website</a>")
...
class ScheduleHandler(BaseHandler):
    ...
    def renderSchedule(self, errorMsg="", rfn=""):
            ...
            for rot in rotation:
                if rot.rotation_pos > 0:
                    rotationList = rotationList + \
                        str(rot.rotation_pos) + ". " + rot.name + "<br>"
        ...
        houseName = 'bose'
        logging.info("Creating " + houseName + " row")

        currentRentals = Rental.all().filter('startDate > ', dateFilter).filter(
            'houseName', houseName).order('-startDate')
        logging.info(currentRentals.count())
        boseRentalRow = u.createScheduleRow(
            currentRentals, houseName, startDate, endDate, "Bose House", self, public)
        ...
        currentRentals = Rental.all().filter('startDate > ', dateFilter).filter(
            'houseName', houseName).order('-startDate')
        logging.info(currentRentals.count())
        minkRentalRow = u.createScheduleRow(
            currentRentals, houseName, startDate, endDate, "Mink Point", self, public)
        ...
        currentRentals = Rental.all().filter('startDate > ', dateFilter).filter(
            'houseName', houseName).order('-startDate')
        logging.info(currentRentals.count())
        mainRentalRow = u.createScheduleRow(
            currentRentals, 'main', startDate, endDate, "Main House", self, public)
        ...
        context = {'userTurnToSchedule': 'true', 'startDatesSelect': startDates,
                   ...
                   'rotationList': rotationList, 'public': public, 'rates': rates, 'year': year}

        u.render('schedule.html', context, self,
                 False, "rental/schedule/", public)

    def get(self):
        self.renderSchedule()

    def post(self):
        logging.info("POST to Schedule" + self.request.get('startDate'))

        rentalExists = False

        errorMsg = ""
        sDate = datetime.strptime(self.request.get('startDate'), "%m/%d/%Y")
        eDate = datetime.strptime(self.request.get('endDate'), "%m/%d/%Y")
        ...
        while sDate < eDate:
            ...
            rental = query.get()
            logging.info("Checking if rental exists for " + houseName +
                         " starting on " + self.request.get('startDate'))
            if rental:
                rentalExists = True
                errorMsg = "This house is already rented for at least part of that time by " + \
                    rental.renterName
        ...
        mail.send_mail(sender="Lac Des Iles Web <gewing@gmail.com>",
                       ...
                       body="""Choice Saved for """ + self.request.get('renterName') + """ in """ + houseName + """ from """ +
                       self.request.get('startDate') + """ to """ + self.request.get('endDate') + """.""")
